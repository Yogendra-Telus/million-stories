...
import { POST_RESET_PASSWORD } from '../../store/actions/ActionTypes';
...
const ResetPasswordModal = props => {
  ...
  const handleResetPassword = values => {
    props
      .resetPassword({ resetToken: props.resetToken, newPassword: values.newPassword })
      .then(successResponse => {
        if (successResponse.type === POST_RESET_PASSWORD) {
          message.success(Constants.passwordResetCaption);
          props.history.push('/');
        }
      })
      .catch(error => {
  ...
  return (
    <div className="forgot-modal">
      <CustomModal
        ...
        onCancel={toggle}
        submit={submitValues => handleResetPassword(submitValues)}
        visible={modal}
...
const mapDispatchToProps = dispatch => ({
  resetPassword: authData => dispatch(actionCreator.postResetPassword(authData)),
});

export default connect(
  null,
  mapDispatchToProps
)(withRouter(withFormik({ validationSchema: validateNewPassword })(ResetPasswordModal)));
